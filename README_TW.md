...
- 你一天要在 Slack、Gmail、LINE 之間切換，但語音輸入出來的東西每個地方都長一樣，語氣和格式還是得自己改
- 你打字打到手腕開始不舒服，不想再多一個每月 $12 的訂閱費
- 你在 VSCode、Terminal 或 AI CLI 工具裡工作，不希望語音輸入的結果像在傳訊息
- 你的音訊不想送到任何人的雲端——需要的是可以自己驗證的東西，不是廠商說「我們很安全」就算數
- 你開過很多會，結束後什麼都沒記下來，需要一個不用分心就能跑的即時轉錄
...
- 語音編輯：選取文字後按 `Ctrl+Option+Z`，說出指令（「翻譯成英文」、「改短一點」），AI 改寫後自動貼回
- 會議模式：持續錄音轉錄，邊說邊存成筆記檔案 — 適合不想分心的長時間通話或課程
- Silero VAD 靜音過濾（選配，需另外下載模型）
- 自訂詞典：加入的詞彙會注入 Whisper 提示詞和 LLM 上下文，讓專有名詞不再被亂改
